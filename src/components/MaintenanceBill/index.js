...
class WaterBill extends Component {
  ...
  handleSubmit(e) {
    ...
    this.fetchBillList(this.htmlElement("building").value,this.htmlElement("room").value,
      ...
      this.htmlElement("startEnd").value,this.htmlElement("dueStart").value,this.htmlElement("dueEnd").value,this.htmlElement("paidStart").value,this.htmlElement("paidEnd").value
    )
  }
  handleAllBillButton() {
  ...
  fetchBillList(buildingid,room,floor,statusid,start,startend,due,dueend,paid,paidend) {
    var self = this
    axios.get(`/api/bill/maintenance/buildingid/${buildingid!==""?buildingid:"%20"}/roomno/${room!==""?room:"%20"}/floor/${floor!==""?floor:"%20"}/statusid/${statusid!==""?statusid:"%20"}/start/${start!==""?start:"%20"}/startend/${startend!==""?startend:"%20"}/due/${due!==""?due:"%20"}/dueend/${dueend!==""?dueend:"%20"}/paid/${paid!==""?paid:"%20"}/paidend/${paidend!==""?paidend:"%20"}`)
    .then(function (response) {
      console.log(response.data)
            ...
            <div className="col-sm-3">
              ...
              <form onSubmit={(e)=>this.handleSubmit(e)}>
                <div className="form-row mt-3">
                  ...
                  <input id="paidStart" className="form-control col-6" type="date" placeholder="Paid date"/>
                  <input id="paidEnd" className="form-control col-6" type="date" placeholder="Paid date"/>
                </div>
              </form>
